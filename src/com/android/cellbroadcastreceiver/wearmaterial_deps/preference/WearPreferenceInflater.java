...
/**
 * Forked from {@link androidx.preference.PreferenceInflater}. This allows inflation of
 * wear-specific preferences.
 */
@SuppressWarnings("nullness") // Suppress nullness checks, as this is forked code.
class WearPreferenceInflater {
  ...
  public WearPreferenceInflater(Context context, PreferenceManager preferenceManager) {
    ...
    setDefaultPackages(
        new String[] {
          // A fake prefix for Wear-related preferences
          WearPreference.class.getPackage().getName() + ".Wear",
  ...
  /**
   * Inflate a new item hierarchy from the specified xml resource. Throws InflaterException if there
   * is an error.
   *
  ...
  private PreferenceGroup onMergeRoots(PreferenceGroup givenRoot, PreferenceGroup xmlRoot) {
    ...
    if (givenRoot == null) {
      new PreferenceReflectionHelper(xmlRoot).onAttachedToHierarchy(preferenceManager);
      return xmlRoot;
    } else {
      return givenRoot;
    }
   ...
   */
  private Preference createItem(String name, @Nullable String[] prefixes, AttributeSet attrs)
   ...
   * name. Override it to handle custom item objects. If you override this in your subclass be sure
   * to call through to super.onCreateItem(name) for names you do not recognize.
   *
    ...
    try {
      ...
      final InflateException ie =
          new InflateException(
              attrs.getPositionDescription() + ": Error inflating class (not found)" + name);
      ie.initCause(e);
      throw ie;
